...
static PT_THREAD (protothread_fern(struct pt *pt))
{
    ...
    while(1) {
        ...
        static char sleeptime_fern_str[40];
        sprintf(sleeptime_fern_str, "%d", (int)sleeptime_fern) ;
        setCursor(400, 20) ;
        setTextSize(2) ;
        writeString(sleeptime_fern_str) ;
        setCursor(400,40);
        setTextSize(2) ;
        writeString(freqtext) ;
        tree_x += 260;
