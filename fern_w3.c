...
int main() {
    ...
    int num_trees = 3;
    int tree_x = 160;
    int tree_y1, tree_y2, tree_y3;
    while(true) {
        // draw slope
        tree_y1 = rand() % 100;
        tree_y2 = tree_y1 + (rand() % 100 - 40);
        tree_y3 = tree_y2 + (rand() % 100 - 40);
        drawLine(0, 480, tree_x, 480-tree_y1, WHITE);
        drawLine(tree_x, 480-tree_y1, tree_x * 2, 480-tree_y2, WHITE);
        drawLine(tree_x * 2, 480-tree_y2, tree_x*3, 480-tree_y3, WHITE);
        drawLine(tree_x*3, 480-tree_y3, 640, 480-tree_y3,WHITE);

        // generate three trees
        for(int i = 0; i < num_trees; i++){
            // load current tree y coordinate
            int tree_y_current;
            switch (i) {
                case 0:
                    tree_y_current = tree_y1;
                    break;
                case 1:
                    tree_y_current = tree_y2;
                    break;
                case 2:
                    tree_y_current = tree_y3;
                    break;
                default:
                    tree_y_current = 0;
            }
            ...
            for (int leaf = 0; leaf<max_l; leaf++) {
                ...
                for (int i=0; i<max_count; i++) {
                    ...
                    // draw both leaves
                    drawPixel((x_shrinked_left >>15) + x_offset + tree_x, y_offset - tree_y_current -(y_shrinked_left >>15), GREEN) ;
                    drawPixel((x_shrinked_right>>15) + x_offset + tree_x, y_offset - tree_y_current -(y_shrinked_right>>15), GREEN) ;
                    sleep_us(10);
                }
